...
Public Class frmFiniquito
    ...
    Private Sub btnRecalcular_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRecalcular.Click
        ...
        If cTipar = "F" Then
            ...
            nSubTotalFactura = Round(nSaldoEquipo - nImpDG - nImpRD - nIvaRD + nSaldoSeguro + nSaldoOtros + nInteresTOT + nComision + nSeguroVida + nFega, 2)
            nIvaFactura = Round(nIvaCapital - nIvaDG + nIvaInteresTOT + nIvaComision + nIvaFega, 2)
        ...
        ElseIf cTipar = "R" Then
            ...
            nSubTotalFactura = Round(nSaldoEquipo - nImpDG - nIvaDG - nImpRD - nIvaRD + nSaldoSeguro + nSaldoOtros + nInteresTOT + nComision + nSeguroVida + nFega, 2)
            nIvaFactura = Round(nIvaCapital + nIvaInteresTOT + nIvaComision + nIvaFega, 2)
            nTotalFactura = Round(nSubTotalFactura + nIvaFactura, 2)
        ...
        ElseIf cTipar = "S" Then
            ...
            nSubTotalFactura = Round(nSaldoEquipo + nSaldoSeguro + nSaldoOtros + nInteresTOT + nComision + nSeguroVida + nFega, 2)
            nIvaFactura = Round(nIvaInteresTOT + nIvaComision + nIvaFega, 2)
            nTotalFactura = Round(nSubTotalFactura + nIvaFactura, 2)
    ...
    Private Sub btnAplicar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAplicar.Click
        ...
        If nTotalNota > 0 Then
            ...
            stmWriter2.WriteLine("H1|" & FECHA_APLICACION.ToShortDateString & "|PUE|" & TaQUERY.SacaInstrumemtoMoneSAT(CmbInstruMon.SelectedValue) & "|" & cCheque & "|" & dtpFechaPago.Value.ToShortDateString)

            cRenglon2 = "H3|" & cCliente & "|" & Mid(cAnexo, 1, 5) & "/" & Mid(cAnexo, 6, 4) & "|C|" & nNota & "|" & Trim(cDescr) & "|" &
                Trim(cCalle) & "|||" & Trim(cColonia) & "|" & Trim(cDelegacion) & "|" & Trim(cEstado) & "|" & cCopos & "|" & cCuentaPago & "|" & cFormaPago & "|MEXICO|" & Trim(cRfc) & "|M.N.|" &
                "|NOTA DE CREDITO|" & cCliente & "|LEANDRO VALLE 402||REFORMA Y FFCCNN|TOLUCA|ESTADO DE MEXICO|50070|MEXICO|" & cAnexo & "|000|"
